...
* mention raised hand function on zoom
...
* precisely what we're worried about right now and a major reason the big provincial population centers (toronto and peel) have been moved to lockdown

* if hospitals were to be overwhelmed...
...
???
* covid-19 patients for one, of course

* but also non-covid-19 patients
  * there are many common (and urgent) reasons people get hospitalized every day; here are some examples
...
???
* there are also emergency and routine surgeries
...
* bottom line: a *lot* more is at stake if hospitals were to be overrun besides the outcomes of COVID-19 patients
...
* so going back to the two questions that we're very interested in for COVID, i'd posit that we need to add a question in between:
...
* not just who is getting sick and who is dying, but who is getting _very_ sick

* remember: huge variance in outcomes due to heterogeneity in the population. why? what kind of heterogeneity?
...
* some heterogeneties that have been linked to the variance in COVID severity include...
  ...
  - employment conditions (is this a person who works from home or works in a high-risk setting for contracting COVID? do they have access to regular PPE? if they get exposed, will they be exposed to a high viral load?)
  - race (see correlations with above points)
  - **age**

* this last one is especially interesting because age is readily available in the data (compared with the other attributes above), as well as in clinical settings
...
# **what is the relationship between _age_ and COVID-19 outcomes?**
...
* while we've surpassed 100K known infections, a number of these are still in progress, so we look at resolved KIs
  * resolved = record marked as "resolved" or "fatal"
  * that way, we know the illness has run its course and we can tally outcomes
...
* after shutdown, we started a phased reopening, that seemed to be going well for a while
  * note: the phases here roughly indicate where the most densely populated regions were at the time (reopening hasn't been uniform across the province)
...
* so how do resolved known infection counts break down by age?
...
* interestingly, while in adults, we see roughly an even proportion of tests turning up positive, in kids we have fewer positives than we might expect compared to the number of negatives (i.e. given the total number of tests administered)
...
* note that deaths here are faceted by whether or not we also have a record of hospitalization for that person
  ...
  * can't infer hospitalizations from deaths---don't seem to be strongly correlated
...
* here is the distribution of hospitalizations (and other hospital outcomes) alongside deaths
  * (these are **counts** because that's what determines pressure on the hospital system)
...
???
* when both the first waves and second waves occurred, the testing guidelines selected for sufficiently symptomatic individuals (we were doing untargetted asymptomatic testing in the summer but stopped to protect testing capacity as the second wave started)
...
* **probability of hospitalization given known infection** is likely an _overestimate_ for the general population
  * but probability of survival given hospitalization is not
